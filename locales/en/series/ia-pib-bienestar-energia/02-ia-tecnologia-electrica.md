...
## 1. What compute means physically

Every useful AI workload ultimately converts electricity into computation and then removes the heat created by that computation.

{{ include_html("snippets/ia-pib-bienestar-energia/02-energia-electricidad.html") }}

Two phases have different demand profiles. **Building a model** concentrates a large amount of compute into a finite period. **Serving a model** repeats smaller workloads continuously across many users and applications.

{{ include_html("snippets/ia-pib-bienestar-energia/02-training-inference.html") }}

As adoption grows, serving can become the larger lifetime workload because a small cost is multiplied by large volume and long operating periods.

---

## 2. Workload type changes the cost

A short text response, long-context reasoning, image generation, audio processing and video generation do not use the same amount of compute. Model size, precision, hardware generation, batching, context length, output length and modality all matter.

{{ include_html("snippets/ia-pib-bienestar-energia/02-multimodal.html") }}

This is why there is no single universal value for “energy per AI query”. Any useful estimate must state what model, workload and infrastructure assumptions it represents.

---

## 3. Efficiency can coexist with rising total demand
...
{{ include_html("snippets/ia-pib-bienestar-energia/02-rebound-efecto.html") }}

The engineering question is therefore whether efficiency improves faster than aggregate demand expands.

---

## 4. The physical bottlenecks
...
Large data centers require substantial reliable electrical capacity and supporting facility infrastructure. The IEA estimated global data-center electricity consumption at **415 TWh in 2024** and described substantially higher scenarios for 2030 as both AI and conventional data workloads expand ([IEA, 2025](https://www.iea.org/reports/energy-and-ai)).

{{ include_html("snippets/ia-pib-bienestar-energia/02-proyeccion-demanda.html") }}

### Hardware

Accelerators also depend on a wider semiconductor and networking supply chain. Increasing electricity capacity without the corresponding computing hardware does not create useful compute, and the reverse is also true.

{{ include_html("snippets/ia-pib-bienestar-energia/02-fabricacion-chips.html") }}

### Cooling and resource accounting

The accelerator is only part of the facility. Cooling, networking, storage and power conversion add overhead, while hardware manufacturing and replacement add lifecycle costs that are separate from operational electricity.
...
Compute demand is concentrated in particular places rather than spread evenly across a grid. Large sites value electricity availability, facility capacity and network connectivity, while serving infrastructure also values proximity to users. The same global AI service can therefore depend on several different local infrastructure conditions.

---

## 6. A better measurement stack
...
!!! tip "Continue the path"
    Return to the [series overview](./00_presentacion_serie.md). Chapter 3 connects this physical infrastructure to the measurement problem: GDP is not the same thing as well-being.
