...
| Testing / evals | Local/CI behavioral tests; text-mode conversational simulations on LiveKit Cloud | Pipecat Evals: text/audio scenarios over an eval transport; audio exercises STT/LLM/TTS, not the production transport | You design and maintain the harness, fixtures, fakes, replay, and judges |
...
[^livekit-testing]: LiveKit, [Testing and evaluation](https://docs.livekit.io/agents/start/testing/) and [Agent simulations](https://docs.livekit.io/agents/start/testing/simulations/). The test framework checks turn-level messages, tools, and handoffs locally/in CI. Complete-conversation simulations run on LiveKit Cloud, are beta/Python-only, and are currently text-only; they do not exercise STT/TTS or the full audio pipeline.
