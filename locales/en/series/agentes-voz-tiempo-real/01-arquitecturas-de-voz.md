...
| Testing / evals | Local/CI behavioral tests; end-to-end simulations on LiveKit Cloud | Built-in Pipecat Evals: text/audio scenarios, assertions, tool calls, interruptions, and timing budgets | You design and maintain the harness, fixtures, fakes, replay, and judges |
...
**Testing and evals are another ownership surface.** LiveKit Agents includes a test framework for messages, tool calls, handoffs, and multi-turn conversations that can run locally or in CI; its agent simulations exercise complete conversations and run on LiveKit Cloud.[^livekit-testing] Pipecat now includes **Pipecat Evals** as a first-party system: it runs the real agent through an eval transport, accepts YAML/Python scenarios, supports text and audio modes, semantic assertions, function calls, interruptions, and per-event `within_ms` budgets.[^pipecat-evals] That removes much of the work of building a basic harness, but the application still owns scenario selection, judges, business metrics, tool fixtures, and success criteria. Vanilla gives you complete control over clocks, fake transports, replay, and judges, in exchange for building and maintaining all of those surfaces.
...
[^livekit-testing]: LiveKit, [Testing and evaluation](https://docs.livekit.io/agents/start/testing/). Behavioral test framework for messages, tool calls, and conversations; tests run locally/in CI while complete-scenario agent simulations run on LiveKit Cloud.
...
[^pipecat-evals]: Pipecat, [Pipecat Evals](https://docs.pipecat.ai/pipecat/evals/overview). First-party evaluation system that runs the real agent through an eval transport and harness, with YAML/Python scenarios, text/audio modes, semantic assertions, function calls, interruptions, and event-level timing budgets.
