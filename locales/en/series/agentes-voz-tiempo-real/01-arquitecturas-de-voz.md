...
[^livekit-fallback]: LiveKit, [Fallback strategies](https://docs.livekit.io/agents/logic/fallback-strategies/). Separates the in-process `Agent Fallback Adapter` for STT/TTS/LLM from the managed LiveKit Inference fallback for STT/TTS, and documents partial-output guards: TTS does not switch after audio has been pushed and LLM responses are not restarted by default after text or tool calls have streamed.
[^livekit-errors]: LiveKit, [Events and error handling](https://docs.livekit.io/reference/agents/events/). `AgentSession` emits `ErrorEvent` for STT/LLM/TTS/realtime failures and exposes `recoverable` to distinguish automatic recovery from terminal failure after retries are exhausted.
