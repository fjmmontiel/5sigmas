...
This chapter dismantles the main misconception about orbital data centers: that the cold of space makes cooling free. By the end, we will understand why rejecting heat remains the major design problem, where the real advantage of orbital solar energy lies, and what limits the connection to Earth imposes.

!!! info "Prerequisites"
    This chapter assumes that you know the concepts introduced in [Chapter 1 — Why now](./01-por-que-ahora.md).

The previous chapter identified why terrestrial infrastructure has real frictions and why space has entered the discussion as an alternative for certain use cases. Before asking whether that leap makes sense, it is worth clearing up a misleading intuition: that, because space is cold, cooling servers there should be easy.
...
Those radiators also do not live in a friendly environment. If they receive too much sunlight, they can absorb as much energy as—or more than—they are trying to reject. And because they are large, deployed surfaces, they are also more exposed to small debris and micrometeoroids that cannot even be tracked precisely ([NASA][r1], [LoadPath][r2], [NASA Orbital Debris][r10]).
...
That does not make energy free. Supporting a multi-megawatt installation requires thousands of square metres of panels and tens of tonnes of additional hardware. The economic promise only begins to make sense if the cost of launching mass to space falls substantially and if the cooling system does not consume that advantage ([Starcloud][r3], [Space Investments][r4]).

There is also degradation. Radiation and the space environment gradually damage coatings and materials. The good news is that, with the right design, that wear appears controllable enough to project long service lives for the panels ([Starcloud][r3]).
...
The way to soften that limit is to deploy constellations: many coordinated satellites instead of a single one. But that multiplies the number of launches, cost and complexity. The alternative is satellites that remain continuously above the same point on Earth. They solve continuity, but at the cost of greater communication delay, too high for many interactive applications ([Space Investments][r4]).
...
- Redundancy has to be heavily provisioned from the start, because there is no easy way to expand it later.
