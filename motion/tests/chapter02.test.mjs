...
import {CHAPTER02} from '../src/seguridad/chapter02-data.mjs';
import {beatState,validateBeatScene,expectedBeatEvents} from '../src/seguridad/semantic-beats.mjs';
const copy=x=>structuredClone(x);
for(const l of ['es','en'])for(const s of CHAPTER02.scenes)test(`${s.id}/${l} timing/retention/seek`,()=>{
...
test('decoration does not satisfy a semantic action',()=>{const s=copy(CHAPTER02.scenes[0]);s.beats[1].action='opacity';assert.throws(()=>validateBeatScene(s,'es'),/SEMANTIC/)});
test('unchanged before/after fails',()=>{const s=copy(CHAPTER02.scenes[0]);s.beats[1].expected.after=copy(s.beats[1].expected.before);assert.throws(()=>validateBeatScene(s,'es'),/STATE_TRANSITION/)});
test('text drift fails',()=>{const s=copy(CHAPTER02.scenes[0]);s.text.en+=' Missing cue.';assert.throws(()=>validateBeatScene(s,'en'),/TEXT_DRIFT/)});
test('out of order action fails',()=>{const s=copy(CHAPTER02.scenes[0]);s.beats[1].at=.1;assert.throws(()=>validateBeatScene(s,'en'),/READING|ORDER|HOLD/)});
test('short hold fails',()=>{const s=copy(CHAPTER02.scenes[0]);s.beats[1].settledAt=s.beats[2].at-.1;assert.throws(()=>validateBeatScene(s,'en'),/HOLD/)});
