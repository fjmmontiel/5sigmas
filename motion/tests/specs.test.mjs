...
const byName=name=>specs.find(row=>row.name===name);
...
test('all six Spanish modelos-razonadores specs satisfy the deterministic v4 contract',()=>{
  assert.equal(specs.length,6,'the release unit must contain exactly the six authored Spanish chapters');
...
test('failure chapter exposes the intended mechanisms instead of decorative fallback scenes',()=>{
  const row=byName('02-fallos.es.json');
...
test('latency chapter has five semantic mechanisms and source-faithful timing claims',()=>{
  const row=byName('04-latencia-streaming.es.json');
  assert.ok(row,'04-latencia-streaming.es.json must be present');
  ...
  assert.match(routing.paragraphs.join(' '),/más de dos veces sin degradar la calidad medida/i);
});

test('risk chapter keeps sourced attack mechanisms distinct from illustrative controls',()=>{
  const row=byName('05-riesgos.es.json');
  assert.ok(row,'05-riesgos.es.json must be present');
