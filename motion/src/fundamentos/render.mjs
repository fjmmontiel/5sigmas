...
export function renderFundamentosFrame(canvas,register,jobId,timeSeconds,{reducedMotion=false}={}){
  ...
  drawFundamentosEditorial(ctx,FUNDAMENTOS_THEME,layout,{series:job.locale==='es'?'Fundamentos de IA e IA generativa':'AI and Generative AI Foundations',title:scene.title,body:scene.body,visibleBody:scene.timeline.visible_body,sceneIndex:scene.index,sceneCount:5,timeSeconds:frame.timeSeconds,totalSeconds:75,family:scene.perceptualFamily});
  ...
  return Object.freeze({jobId:job.id,locale:job.locale,orientation:job.orientation,scene:scene.conceptId,sceneIndex:scene.index,timeSeconds:frame.timeSeconds,localSeconds:frame.localSeconds,bodySize:layout.bodySize,mechanismScale:m.scale,metrics:layout.metrics,issues:Object.freeze(issues),family:scene.perceptualFamily,rendererFamily:scene.rendererFamily,topology:scene.topology,mechanismId:scene.mechanism.mechanismId,visualStyle:scene.mechanism.style,reducedMotion,cueTimelineVersion:scene.timeline.version,currentCueId:scene.timeline.current_cue_id,visibleSentenceIds:scene.timeline.visible_sentence_ids,visibleBody:scene.timeline.visible_body});
}
