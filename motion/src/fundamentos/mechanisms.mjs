...
export function compileFundamentosMechanism(concept,{orientation='horizontal',localSeconds=0,durationSeconds=15,reducedMotion=false}={}){
  ...
  const nodes=concept.cues.map((cue,index)=>Object.freeze({id:`cue-${index}`,cue,position:positions[index],active:reducedMotion||localSeconds>=cue}));
  const edges=[];
  if(['hub','attention-web','overlap-map','boundary-radar','lattice'].includes(style)){
    for(let i=0;i<nodes.length;i++)edges.push(Object.freeze({from:'core',to:nodes[i].id,active:reducedMotion||nodes[i].active}));
  } else if(['comparison-grid','parallel-pipeline','parallel-lanes','output-space','trajectory-gap','distribution'].includes(style)){
    for(let i=0;i+1<nodes.length;i+=2)edges.push(Object.freeze({from:nodes[i].id,to:nodes[i+1].id,active:reducedMotion||nodes[i+1].active}));
  } else {
    for(let i=0;i<nodes.length-1;i++)edges.push(Object.freeze({from:nodes[i].id,to:nodes[i+1].id,active:reducedMotion||nodes[i+1].active}));
  }
