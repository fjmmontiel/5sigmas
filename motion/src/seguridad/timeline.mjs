...
export function seguridadTextState(text,conceptId,localSeconds,durationSeconds,{reducedMotion=false,locale='es'}={}){
  ...
  const cues=ranges.map((range,index)=>{
    ...
    return Object.freeze({id:`${conceptId}.beat-${String(index+1).padStart(2,'0')}`,sentence_id:`${conceptId}.beat-${String(index+1).padStart(2,'0')}`,concept_id:conceptId,visual_target_id:`${conceptId}.phase-${index+1}`,action:'read_then_show_corresponding_phase',paragraph:0,range:Object.freeze(range),visible,alpha,guideStrong:true,guideAlpha:reducedMotion?0:.92*emphasisIn*(1-emphasisOut),emphasis:reducedMotion?0:.90*emphasisIn*(1-emphasisOut),status:local<start?'future':local<next?'active':'read',text_at:start,visual_at:visualAt,locale});
  });
