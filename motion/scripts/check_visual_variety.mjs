...
import {visualFamilyInventory,selectEditorialRenderer} from '../src/render/mechanisms/editorial.mjs';
...
export function validateVisualVariety(){
  ...
  for(const file of files){
    ...
    for(const row of visualFamilyInventory(spec)){
      ...
      const genericAllowed=new Set(['candidate-evidence','search-tree','token-clock']);
      if(!genericAllowed.has(row.family))assert.equal(typeof selectEditorialRenderer(spec,scene),'function',`${file}:${row.scene} must have a scene-specific semantic renderer`);
  ...
  return {series:'modelos-razonadores',sceneCount:inventory.length,repeatCap,families:Object.fromEntries([...counts].sort()),inventory,layout:EDITORIAL_LAYOUT_CONTRACT};
}
