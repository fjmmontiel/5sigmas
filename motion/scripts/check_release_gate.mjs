...
for(const [key,row] of Object.entries(status.outputs)){
  ...
  assert.equal(row.technical_golden,allGates,`${key}: technical_golden must be exactly the conjunction of every required gate`);
...
const blocked=Object.entries(status.outputs).filter(([,row])=>!row.technical_golden).map(([key,row])=>({key,missing:status.requiredGates.filter(gate=>!row[gate])}));
const ready=golden.length===expected.length&&migration.status==='golden';
console.log(JSON.stringify({unit:migration.unit,accent:status.accent,technicalGolden:`${golden.length}/${expected.length}`,goldenExamples:exemplars,ready,blocked},null,2));
if(requireReady&&!ready){console.error('RELEASE BLOCKED: one PR is allowed only when all 12 localized outputs are technically GOLDEN and migration.status is golden.');process.exit(2);}
