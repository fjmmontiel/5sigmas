...
@lru_cache(maxsize=8)
def _load_article_audio_index(locale: str = "es") -> dict:
    normalized = str(locale or "es").strip().lower()
    path = SPANISH_ARTICLE_AUDIO_INDEX if normalized == "es" else LOCALES_ROOT / normalized / "article_audio.yml"
    if not path.exists():
        return {}
    try:
...
def _ui(config) -> dict:
    if _locale(config).startswith("en"):
        return {
            ...
            "audio_eyebrow": "Article audio",
            "audio_title": "Listen to this article",
            "audio_voice": "Narrated with the {voice} voice.",
            "unsupported_audio": "Your browser does not support the audio element.",
        }
    return {
        "caption_lang": "es",
        ...
        "audio_eyebrow": "Audio local",
        "audio_title": "Escucha el artículo",
        "audio_voice": "Narrado con la voz {voice}.",
        "unsupported_audio": "Tu navegador no soporta el elemento de audio.",
    }
...
def _audio_url(audio_file: str, config) -> str:
    if re.match(r"^https?://", audio_file, re.IGNORECASE):
        return audio_file
    relative = audio_file.strip().lstrip("/")
    ...
    if relative.startswith(expected_prefix):
        return f"/{relative}"
    return f"/{expected_prefix}{relative}"


def _render_article_audio(page, video_dom_id: str, config) -> str:
    locale = _locale(config)
    entry = _load_article_audio_index(locale).get(page.file.src_path)
    ...
    if not audio_file:
        return ""
    ui = _ui(config)
    title = _esc(entry.get("title") or ui["audio_title"])
    voice = _esc(entry.get("voice_label") or entry.get("voice") or "Kokoro")
    mime = MIME_TYPES.get(Path(audio_file).suffix.lower(), "audio/wav")
    audio_url = _esc(_audio_url(audio_file, config))
    detail = _esc(ui["audio_voice"].format(voice=voice))
    return (
        f'<div class="s5-article-audio" data-video-id="{video_dom_id}">\n'
        '  <div class="s5-article-audio__meta">\n'
        f'<p class="s5-article-audio__eyebrow">{_esc(ui["audio_eyebrow"])}</p>\n'
        f'<p class="s5-article-audio__title">{title}</p>\n'
        f'<p class="s5-article-audio__detail">{detail}</p>\n</div>\n'
        f'<audio controls preload="none" class="s5-article-audio__player" data-audio-role="podcast"><source src="{audio_url}" type="{mime}">{_esc(ui["unsupported_audio"])}</audio>\n</div>'
    )
...
def on_post_page(output: str, page, config, **kwargs) -> str:
    ...
    audio_html = _render_article_audio(page, video_dom_id, config)
