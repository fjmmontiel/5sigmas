(() => {
  ...
  const CONTRACTS = [
    {
      key: '01', selector: '.s5v-inference-phases', scroll: '.s5v-inference-phases__scroll', height: 460,
      ...
      nodes: [
        ['request', 15, 11, 'Petición', 'Request', 'input'], ['queue', 50, 11, 'Cola', 'Queue', 'decision'], ['prefill', 85, 11, 'Prefill', 'Prefill', 'compute'],
        ['first', 85, 38, '1ª salida', '1st output', 'outcome'], ['decode', 50, 38, 'Decode', 'Decode', 'compute'], ['kv', 15, 38, 'Estado KV', 'KV state', 'state'],
        ['concurrency', 15, 75, 'Concurrencia', 'Concurrency', 'input'], ['scheduler', 50, 75, 'Scheduler', 'Scheduler', 'decision'], ['goodput', 85, 75, 'Goodput', 'Goodput', 'outcome'],
      ],
      edges: [
        ['e1','request','queue',[],32,6,'',''], ['e2','queue','prefill',[],68,6,'',''], ['e3','prefill','first',[],91,24,'TTFT','TTFT'],
        ['e4','first','decode',[],68,33,'',''], ['e5','prefill','kv',[[85,25],[15,25]],50,21,'crea KV','builds KV'], ['e6','kv','decode',[],32,33,'reutiliza','reuses'],
        ['e7','concurrency','scheduler',[],32,70,'',''], ['e8','scheduler','goodput',[],68,64,'capacidad','capacity'], ['e9','scheduler','queue',[[50,58]],58,55,'presión de cola','queue pressure'],
      ],
    },
    {
      key: '02', selector: '.s5v-kv-paging', scroll: '.s5v-kv-paging__scroll', height: 490,
      ...
      nodes: [
        ['a',15,10,'Req A','Req A','input'], ['b',15,30,'Req B','Req B','input'], ['table',50,20,'Block table','Block table','state'], ['gpu',85,20,'Pool GPU','GPU pool','state'],
        ['finish',85,47,'Fin / release','Finish / release','event'], ['free',50,47,'Páginas libres','Free pages','state'], ['waiting',15,76,'Req C espera','Req C waits','input'], ['scheduler',50,76,'Scheduler','Scheduler','decision'], ['admit',85,76,'Admitir C','Admit C','outcome'], ['host',85,90,'Host tier','Host tier','state'],
      ],
      edges: [
        ['e1','a','table',[],31,10,'',''], ['e2','b','table',[],31,30,'',''], ['e3','table','gpu',[],68,15,'mapa','maps'], ['e4','gpu','finish',[],91,34,'',''],
        ['e5','finish','free',[],68,43,'libera','releases'], ['e6','free','scheduler',[],56,61,'capacidad','capacity'], ['e7','waiting','scheduler',[],32,71,'',''], ['e8','scheduler','admit',[],68,65,'admite','admits'],
        ['e9','admit','table',[[72,62],[72,33]],74,57,'',''], ['e10','gpu','host',[],91,58,'',''],
      ],
    },
    {
      key: '03', selector: '.s5v-quant-parallel', scroll: '.s5v-quant-parallel__scroll', height: 510,
      ...
      nodes: [
        ['bytes',15,10,'Weights + KV','Weights + KV','input'], ['quant',50,10,'Precisión','Precision','decision'], ['kernel',85,10,'Kernel / HW','Kernel / HW','compute'],
        ['local',85,34,'Bytes locales','Local bytes','state'], ['model',15,58,'Modelo','Model','input'], ['placement',50,58,'Modo paralelo','Parallel mode','decision'], ['comms',85,58,'Ops colectivas','Collective ops','compute'],
        ['topology',85,82,'Enlace GPU','GPU link','state'], ['outcome',50,88,'Latencia / memoria / calidad','Latency / memory / quality','outcome'],
      ],
      edges: [
        ['e1','bytes','quant',[],32,5,'',''], ['e2','quant','kernel',[],68,5,'',''], ['e3','kernel','local',[],91,22,'',''],
        ['e4','model','placement',[],32,53,'',''], ['e5','placement','comms',[],68,48,'reparte','splits'], ['e6','comms','topology',[],91,70,'',''],
        ['e7','local','outcome',[[72,44],[72,82]],66,42,'trade-off','trade-off'], ['e8','topology','outcome',[],68,86,'',''],
      ],
    },
    {
      key: '04', selector: '.s5v-reuse-spec', scroll: '.s5v-reuse-spec__scroll', height: 600,
      ...
      nodes: [
        ['prefix',16,8,'Prefix','Prefix','input'], ['lookup',50,8,'Lookup','Lookup','decision'], ['hit',84,8,'HIT','HIT','outcome'], ['reuse',84,22,'Reusar KV','Reuse KV','state'],
        ['miss',50,28,'MISS','MISS','event'], ['prefill',84,36,'Prefill completo','Full prefill','compute'],
        ['draft',16,56,'Draft','Draft','compute'], ['verify',50,56,'Target verify','Target verify','decision'], ['accept',84,49,'ACCEPT','ACCEPT','outcome'], ['commit',84,64,'Commit','Commit','state'],
        ['reject',50,74,'REJECT','REJECT','event'], ['correct',84,79,'Rewind / corregir','Rewind / correct','compute'], ['pressure',50,92,'KV + scheduler','KV + scheduler','state'],
      ],
      edges: [
        ...
        ['e11','correct','verify',[[84,88],[38,88],[38,64]],61,86,'reverifica ↺','reverify ↺'], ['e12','reuse','pressure',[[72,22],[72,88],[50,88]],74,45,'',''], ['e13','commit','pressure',[[72,64],[72,88]],72,82,'',''],
      ],
    },
    {
      key: '05', selector: '.s5v-routing-policy', scroll: '.s5v-routing-policy__scroll', height: 600,
      ...
      nodes: [
        ['request',16,8,'Petición','Request','input'], ['eligible',50,8,'¿Elegible?','Eligible?','decision'], ['cache',84,8,'Response cache','Response cache','decision'], ['hit',84,23,'HIT → salida','HIT → output','outcome'],
        ['router',50,32,'Router','Router','decision'], ['placement',84,38,'Placement','Placement','decision'], ['primary',84,52,'Primary','Primary','compute'],
        ['success',50,56,'SUCCESS','SUCCESS','outcome'], ['fallback',84,67,'Fallback gate','Fallback gate','decision'], ['alt',84,82,'Fallback model','Fallback model','compute'],
        ['telemetry',50,84,'Telemetry','Telemetry','state'], ['policy',16,67,'Policy update','Policy update','state'],
      ],
      edges: [
        ['e1','request','eligible',[],32,4,'',''], ['e2','eligible','cache',[],68,4,'',''], ['e3','cache','hit',[],90,16,'',''], ['e4','cache','router',[[70,20],[50,20]],60,19,'',''],
        ['e5','router','placement',[],68,30,'',''], ['e6','placement','primary',[],90,45,'',''], ['e7','primary','success',[],68,52,'ok','ok'], ['e8','primary','fallback',[],90,60,'fail','fail'],
        ['e9','fallback','alt',[],90,75,'',''], ['e10','success','telemetry',[],54,70,'',''], ['e11','alt','telemetry',[],68,82,'',''], ['e12','telemetry','policy',[[32,84],[16,84]],33,78,'aprende','learns'], ['e13','policy','router',[[16,48],[38,48],[38,32]],27,45,'feedback','feedback'],
      ],
    },
    {
      key: '06', selector: '.s5v-benchmark-boundary', scroll: '.s5v-benchmark-boundary__scroll', height: 540,
      ...
      nodes: [
        ['workload',15,10,'Workload','Workload','input'], ['sut',50,10,'SUT / queue','SUT / queue','compute'], ['response',85,10,'Salidas','Outputs','state'],
        ['clock',15,38,'Client clock','Client clock','input'], ['latency',50,38,'TTFT / TPOT / E2E','TTFT / TPOT / E2E','state'], ['goodput',85,38,'Goodput + SLO','Goodput + SLO','outcome'],
        ['power',15,66,'Power + cost','Power + cost','input'], ['account',50,66,'∫P(t)dt + ledger','∫P(t)dt + ledger','state'], ['denom',85,66,'Cost / energy / task','Cost / energy / task','outcome'],
        ['sweep',24,90,'Barrido de carga','Load sweep','decision'], ['report',76,90,'Región operativa','Operating region','outcome'],
      ],
      edges: [
        ...
        ['e8','goodput','report',[[85,78]],83,75,'',''], ['e9','denom','report',[[85,82]],82,82,'',''], ['e10','sweep','report',[],50,82,'región útil','operating region'], ['e11','sweep','sut',[[24,78],[24,24],[50,24]],19,53,'varía carga ↺','vary load ↺'],
      ],
    },
  ];
  ...
  const boundaryPoint = (center, toward) => {
    ...
    const tx = Math.abs(dx) < 0.001 ? Number.POSITIVE_INFINITY : 14.8 / Math.abs(dx);
    const ty = Math.abs(dy) < 0.001 ? Number.POSITIVE_INFINITY : 6.2 / Math.abs(dy);
