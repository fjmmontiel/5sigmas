(function (root, factory) {
  ...
  function evaluate(raw = {}) {
    ...
    const paths = [
      ...
      path('sensitive-disclosure', privilegedInfluence && input.sensitiveContext && !input.outputSecretFilter, 'confidentiality', !input.sensitiveContext ? 'No sensitive context is modeled.' : input.outputSecretFilter ? 'A deterministic output secret filter blocks this modeled path.' : 'Sensitive context and an outward response path coexist.', ['outputSecretFilter', 'quarantineReader']),
      path('unauthorized-action', privilegedInfluence && input.toolsEnabled && input.writeTools && !input.actionIntentValidation && !input.humanApproval, 'integrity', !input.toolsEnabled || !input.writeTools ? 'No consequential write-capable tool is modeled.' : (input.actionIntentValidation || input.humanApproval) ? 'Intent validation or human approval blocks the modeled action path.' : 'The model can reach write-capable tools without an independent approval boundary.', ['leastPrivilege', 'actionIntentValidation', 'humanApproval', 'quarantineReader']),
    ...
    const controls = [
      ...
      { id: 'leastPrivilege', enabled: input.leastPrivilege, class: 'impact', note: 'Reduces what a compromised agent can access; it does not prevent model steering.' },
      ...
      { id: 'humanApproval', enabled: input.humanApproval, class: 'action', note: 'Requires confirmation before consequential actions.' },
      ...
      { id: 'outputSecretFilter', enabled: input.outputSecretFilter, class: 'output', note: 'Deterministically blocks known secret classes from rendered output; not a complete injection defense.' }
    ];
    ...
    return {
      ...
      caveat: 'This is deterministic reachability modeling, not a probability of compromise. Real prompt-injection resistance depends on model behavior, implementation details, adversarial adaptation and the exact trust boundaries.'
    };
  }
