...
Claude Code documenta que los command hooks se ejecutan con los permisos completos del usuario.[^claude-hooks] Además, su semántica actual de workspace trust no es uniforme por tipo de configuración: en una sesión interactiva retiene los hooks de settings hasta que se acepta el trust del workspace, pero en `claude -p` o una sesión SDK trata la carpeta como trusted para esos hooks, de modo que hooks comprometidos en `.claude/settings.json` pueden ejecutarse aunque el usuario nunca haya aprobado esa carpeta interactivamente.[^claude-hooks] En cambio, las reglas `permissions.allow` y `additionalDirectories` de `.claude/settings.json` que amplían capacidad siguen sin aplicarse en `-p`/SDK si esa carpeta concreta no fue trusted previamente; no aparece diálogo y Claude Code emite un warning.[^claude-permissions] Los hooks de frontmatter de un subagente de proyecto siguen una regla más estricta: desde v2.1.218 requieren workspace trust; versiones anteriores podían ejecutarlos desde carpetas no confiadas.[^claude-hooks]
