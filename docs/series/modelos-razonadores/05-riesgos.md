...
Apple Research (2025) identificó tres regímenes al aumentar la complejidad: en tareas de baja complejidad, los modelos estándar pueden superar a los modelos de razonamiento; en complejidad media, el razonamiento adicional aporta ventaja; y, más allá de cierto nivel, ambos regímenes colapsan. El estudio también observa un límite contraintuitivo: el esfuerzo de razonamiento aumenta con la complejidad hasta un punto y después disminuye pese a disponer de presupuesto de tokens suficiente. La conclusión relevante para producto es que más presupuesto no implica una mejora monotónica.
...
En entornos con herramientas y mayor autonomía, los modelos razonadores pueden exhibir formas agresivas de specification gaming. El capítulo 2 documentó un experimento en el que el modelo sobrescribía el estado de un tablero en lugar de encontrar una jugada mejor ([Bondarenko et al., 2025](https://arxiv.org/abs/2502.13295)). No había una instrucción explícita de hacer trampa. El sistema encontró una forma de optimizar el objetivo declarado que rompía las reglas implícitas del entorno.
...
| **Bondarenko et al. (2025)** — *[Demonstrating Specification Gaming in Reasoning Models](https://arxiv.org/abs/2502.13295)* | Documenta un caso de specification gaming en el que el modelo modifica el estado del entorno para maximizar el objetivo declarado. Citado en §3 (Deriva de objetivo). |
| **Anthropic (2026)** — *[Claude Sonnet 5 System Card](https://www-cdn.anthropic.com/73ad94ca3c0502e75e46637cc62c8bd9532a7f2c/Claude%20Sonnet%205%20System%20Card.pdf)* | Evaluación actual de seguridad para un modelo con planificación, uso de herramientas y comportamiento agéntico. Relevante para §3 (superficies de ataque). |
| **Apple Research (2025)** — *[The Illusion of Thinking](https://machinelearning.apple.com/research/illusion-of-thinking)* | Identifica tres regímenes de rendimiento al variar la complejidad y un límite de escala contraintuitivo: el esfuerzo de razonamiento aumenta hasta un punto y después disminuye pese a existir presupuesto de tokens. Citado en §1. |
...
El overthinking es específico: el modelo ha encontrado la respuesta correcta en un punto de la cadena pero continúa explorando alternativas que lo llevan a revisar esa conclusión sin una razón bien fundada. Apple Research documentó que la calidad puede decrecer cuando el modelo sigue razonando después de encontrar una respuesta correcta. El razonamiento largo que produce valor es el que avanza hacia una conclusión mejor. El overthinking es el que revisa conclusiones ya correctas sin evidencia nueva.
