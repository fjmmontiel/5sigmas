...
[^livekit-errors]: LiveKit, [Events and error handling](https://docs.livekit.io/reference/agents/events/). `AgentSession` emite `ErrorEvent` para errores STT/LLM/TTS/realtime; el objeto de error expone `recoverable`, que indica si la sesión reintenta automáticamente o si el fallo es terminal tras agotar reintentos.
