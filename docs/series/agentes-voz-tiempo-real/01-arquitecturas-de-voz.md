...
| Media y transporte | WebRTC como camino principal; SIP/telephony integrado en el ecosistema LiveKit | Daily, LiveKit, SmallWebRTC, WebSocket y serializers según el caso | WebRTC provider-direct, Media Streams/WebSocket de carrier o SIP/RTP propio; sin agent framework, pero el endpoint elegido decide quién posee la media |
...
| Testing / evals | Tests de comportamiento locales/CI; simulaciones end-to-end en LiveKit Cloud | Pipecat Evals integrado: escenarios text/audio, assertions, tool calls, interrupciones y budgets de timing | Harness, fixtures, fakes, replay y judges los diseñas y mantienes tú |
...
**Testing/evals es otra superficie de ownership.** LiveKit Agents incluye un test framework para mensajes, tool calls, handoffs y conversaciones multi-turn que puede correr localmente o en CI; sus agent simulations ejercitan conversaciones completas y se ejecutan en LiveKit Cloud.[^livekit-testing] Pipecat ya incluye **Pipecat Evals** como sistema first-party: ejecuta el agente real mediante un eval transport, acepta escenarios YAML/Python, soporta modo texto y audio, assertions semánticas, function calls, interrupciones y presupuestos por evento con `within_ms`.[^pipecat-evals] Eso reduce claramente el trabajo de construir un harness básico, pero no elimina la decisión de qué escenarios, judges, métricas de negocio, fixtures de tools ni criterios de éxito representan tu producto. Vanilla conserva control total sobre clocks, transportes fake, replay y judges, a cambio de construir y mantener todas esas superficies.
...
[^livekit-testing]: LiveKit, [Testing and evaluation](https://docs.livekit.io/agents/start/testing/). Test framework para comportamiento, tool calls y conversaciones; los tests corren localmente/CI y las agent simulations de escenarios completos corren en LiveKit Cloud.
...
[^pipecat-evals]: Pipecat, [Pipecat Evals](https://docs.pipecat.ai/pipecat/evals/overview). Sistema first-party de evaluación sobre el agente real: eval transport + harness, escenarios YAML/Python, modos texto/audio, assertions semánticas, function calls, interrupciones y budgets de latencia por evento.
...
[^twilio-media-streams]: Twilio, [Media Streams overview](https://www.twilio.com/docs/voice/media-streams). Documenta streaming de audio crudo de llamadas por WebSocket y streams bidireccionales para conversaciones realtime con asistentes de IA.
[^twilio-media-messages]: Twilio, [Media Streams — WebSocket Messages](https://www.twilio.com/docs/voice/media-streams/websocket-messages). Documenta `media`, `mark` y `clear`, audio μ-law/8 kHz, buffering de playback y confirmación/limpieza de audio reproducido.
