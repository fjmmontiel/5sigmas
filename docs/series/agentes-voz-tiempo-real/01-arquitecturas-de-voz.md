...
Esta arquitectura sólo existe si el proveedor realtime permite una modalidad de respuesta exclusivamente textual. LiveKit lo señala explícitamente como requisito y advierte de que el soporte varía por proveedor.[^livekit-pipelines] En OpenAI Realtime, el contrato de sesión permite pedir `output_modalities: ["text"]`, que fuerza una respuesta sólo textual; no se puede pedir texto y audio simultáneamente en esa misma respuesta.[^openai-realtime-output-modality] `gpt-realtime-2.1`, además, declara entrada/salida de texto y audio y function calling.[^openai-realtime-model] OpenAI marcó el anterior `gpt-realtime` como deprecated el 20 de julio de 2026 y recomienda `gpt-realtime-2.1` antes de su retirada de la API el 20 de enero de 2027.[^openai-realtime-deprecation] Esto demuestra el requisito para ese provider concreto; no debe extrapolarse a cualquier modelo realtime.
...
| Reliability / flow control | `Agent Fallback Adapter` ejecuta fallback STT/TTS/LLM dentro del proceso; LiveKit Inference tiene otro fallback gestionado para STT/TTS. Por defecto no cambia TTS después de enviar audio ni reinicia una respuesta LLM tras emitir texto/tool calls; `ErrorEvent.recoverable` separa recuperación automática de error terminal[^livekit-fallback][^livekit-errors] | `CancelFrame`, `ErrorFrame` upstream y colas por processor son primitivas del core; retry/failover y los límites o políticas drop/block/coalesce dependen del processor, transporte, servicio y aplicación, no de una garantía global[^pipecat-frame-processor] | Tú defines límites y política de colas, idempotencia de retries/tools, propagación de cancelación, reglas tras output parcial, clasificación de errores y aislamiento de failure domains |
...
**1. Voice assistant en browser o móvil.** Si usuarios reales llegan desde redes variables y necesitas media robusta, WebRTC es la base natural. LiveKit Agents encaja bien cuando quieres que rooms, media, agent workers y, si lo eliges, operación gestionada formen un sistema coherente.[^livekit-agents] Pipecat encaja cuando la prioridad es componer processors/providers y quieres seleccionar Daily o LiveKit como transporte sin cambiar la lógica central del pipeline.[^pipecat-transports][^pipecat-livekit] Un camino vanilla/thin también puede ser realmente ligero: el browser puede enviar audio por WebRTC directamente al proveedor y el backend quedarse con autenticación, credenciales efímeras, autorización de tools y estado de negocio.[^openai-realtime-transport] Eso reduce plumbing propio, pero aumenta el acoplamiento al protocolo del proveedor y no elimina ninguna frontera de seguridad server-side.

**2. Agente PSTN.** LiveKit es una opción fuerte cuando quieres terminar SIP dentro del mismo sistema de rooms y despachar agents sobre ese lifecycle.[^livekit-sip] Pipecat es atractivo cuando ya eliges un carrier/streaming API y quieres que serializers, turn strategies, STT/LLM/TTS y tools vivan en un pipeline sustituible; su `FastAPIWebsocketTransport` está orientado precisamente a telefonía y conexiones WebSocket server-side.[^pipecat-telephony] Vanilla/thin no exige operar tu propio gateway SIP/RTP: un carrier puede terminar la llamada y exponer media bidireccional por WebSocket. Twilio Media Streams, por ejemplo, entrega audio de la llamada a tu servidor y permite enviar audio de vuelta.[^twilio-media-streams] Pero esa frontera tiene un contrato propio: en un stream bidireccional sólo recibes el track inbound, hay un único Stream por llamada y el audio se entrega como `audio/x-mulaw`, 8 kHz, mono. El outbound se bufferiza en orden. `mark` informa cuando Twilio ha terminado de reproducir el audio precedente; si envías `clear`, Twilio vacía el buffer y devuelve los `mark` pendientes. Eso permite distinguir audio reproducido por Twilio de audio descartado, pero **no demuestra que la persona lo haya oído físicamente**. Además, tu servidor debe validar `X-Twilio-Signature` para autenticar el origen del stream.[^twilio-media-streams][^twilio-media-messages] En ese diseño Python posee el contrato WebSocket/audio y el estado de llamada de aplicación, mientras Twilio posee la terminación telefónica. Es una opción thin cuando ese codec, esa superficie de control y esa frontera de seguridad son suficientes; si necesitas control que el carrier no expone, debes bajar hacia SIP/RTP u otro endpoint de media.
...
[^livekit-license]: LiveKit Agents, [Python package metadata](https://github.com/livekit/agents/blob/main/livekit-agents/pyproject.toml). Declara el framework `livekit-agents` bajo licencia Apache-2.0.
[^livekit-billing]: LiveKit, [LiveKit Cloud billing](https://docs.livekit.io/deploy/admin/billing/). Documenta metering separado para media WebRTC/SIP, agent deployment, observabilidad e inference y sus unidades de consumo; no se usa aquí para fijar precios, que pueden cambiar.
...
[^livekit-testing]: LiveKit, [Testing and evaluation](https://docs.livekit.io/agents/start/testing/) y [Agent commands](https://docs.livekit.io/reference/developer-tools/livekit-cli/agent/). El test framework es text-based y corre localmente/CI. Las Agent Simulations se ejecutan en LiveKit Cloud; la CLI actual ofrece `lk agent simulate audio`, que recorre STT–LLM–TTS con una pista de audio del usuario simulado y permite introducir ruido de fondo, micrófono degradado y packet loss.
[^livekit-fallback]: LiveKit, [Fallback strategies](https://docs.livekit.io/agents/logic/fallback-strategies/). Separa `Agent Fallback Adapter`, ejecutado dentro del proceso del agent para STT/TTS/LLM, de `Inference Fallback Adapter`, perteneciente al servicio gestionado LiveKit Inference para STT/TTS. Documenta además las protecciones ante output parcial: TTS no conmuta después de enviar audio y LLM no reinicia por defecto después de emitir texto o tool calls.
[^livekit-errors]: LiveKit, [Events and error handling](https://docs.livekit.io/reference/agents/events/). `AgentSession` emite `ErrorEvent` para errores STT/LLM/TTS/realtime y expone `recoverable` para distinguir recuperación automática de un error terminal tras agotar reintentos.
...
[^pipecat-frame-processor]: Pipecat, [`FrameProcessor` source](https://github.com/pipecat-ai/pipecat/blob/main/src/pipecat/processors/frame_processor.py). El core implementa `FrameProcessorQueue` sobre `asyncio.PriorityQueue`, prioriza frames de sistema, procesa cada processor en sus propias tareas y usa `CancelFrame` para cancelación; estas primitivas no definen por sí solas una política de backpressure/retry de producto.
[^pipecat-cascade-s2s]: Pipecat, [Building With OpenAI Audio Models and APIs](https://docs.pipecat.ai/pipecat/features/openai-audio-models-and-apis). Documenta en código tanto un pipeline cascade STT→LLM→TTS como un pipeline speech-to-speech sobre OpenAI Realtime; no implica que cualquier provider soporte half-cascade o full-duplex.
...
[^pipecat-evals]: Pipecat, [Pipecat Evals](https://docs.pipecat.ai/pipecat/evals/overview). Sistema first-party de desarrollo que ejecuta el agente real sobre un eval transport local en lugar de Daily/WebRTC/telefonía. El modo audio sintetiza la entrada, la pasa por el STT real del agente y evalúa su TTS real; cubre STT/LLM/TTS y comportamiento conversacional, no el transporte de producción ni las condiciones reales de red/telefonía.
[^pipecat-license]: Pipecat, [LICENSE](https://github.com/pipecat-ai/pipecat/blob/main/LICENSE). Declara Pipecat core bajo BSD 2-Clause.
[^pipecat-deployment]: Pipecat, [Pipecat](https://www.pipecat.ai/). La superficie oficial separa despliegue self-hosted del core y Pipecat Cloud, que ejecuta, escala y observa agents de forma gestionada; transporte y servicios/modelos siguen siendo componentes elegibles del pipeline.
...
[^pipecat-websocket-auth]: Pipecat, [WebSocket Authentication](https://docs.pipecat.ai/pipecat-cloud/guides/websocket-authentication). Capacidad de Pipecat Cloud para proteger conexiones WebSocket con tokens HMAC de sesión de corta duración; no es una propiedad universal de Pipecat core.
[^openai-realtime-transport]: OpenAI Agents SDK, [Realtime Transport Layer](https://openai.github.io/openai-agents-js/guides/voice-agents/transport/). Documenta audio WebRTC directo browser↔Realtime API con control server-side separado, además de caminos WebSocket, SIP, Twilio y transporte custom; permite distinguir ownership del runtime de ownership del endpoint de media.
...
[^twilio-media-streams]: Twilio, [Media Streams overview](https://www.twilio.com/docs/voice/media-streams). Documenta streams bidireccionales sobre WebSocket para agentes realtime, un único Stream por llamada en modo bidireccional, recepción del track inbound y la obligación de validar `X-Twilio-Signature`.
[^twilio-media-messages]: Twilio, [Media Streams — WebSocket Messages](https://www.twilio.com/docs/voice/media-streams/websocket-messages). Documenta `audio/x-mulaw` a 8 kHz mono, buffering FIFO de media outbound y las semánticas de `mark`/`clear`; un `mark` devuelto refleja finalización de playback en Twilio o limpieza del buffer, no evidencia física de audición en el handset.
[^openai-realtime-output-modality]: OpenAI API Reference, [Realtime call/session `output_modalities`](https://developers.openai.com/api/reference/python/resources/realtime/subresources/calls/methods/accept). Documenta que `output_modalities: ["text"]` solicita una respuesta sólo textual y que no se puede pedir texto y audio simultáneamente en la misma respuesta.
[^openai-realtime-model]: OpenAI, [GPT-Realtime-2.1 model](https://developers.openai.com/api/docs/models/gpt-realtime-2.1). Documenta las modalidades de texto/audio y function calling del modelo realtime recomendado actualmente.
[^openai-realtime-deprecation]: OpenAI, [Deprecations](https://developers.openai.com/api/docs/deprecations), 20 de julio de 2026. Marca `gpt-realtime` como deprecated, fija su retirada de la API para el 20 de enero de 2027 y recomienda `gpt-realtime-2.1` como replacement.
