...
| Testing / evals | Tests de comportamiento locales/CI; simulaciones conversacionales text-only en LiveKit Cloud | Pipecat Evals: escenarios text/audio sobre un eval transport; el modo audio ejercita STT/LLM/TTS, no el transporte de producción | Harness, fixtures, fakes, replay y judges los diseñas y mantienes tú |
...
**Testing/evals es otra superficie de ownership.** LiveKit Agents incluye un test framework para mensajes, tool calls, handoffs y conversaciones multi-turn que puede correr localmente o en CI. Sus agent simulations ejecutan conversaciones completas en LiveKit Cloud, pero actualmente son **text-only**: ejercitan LLM, tools y lógica conversacional, no el pipeline STT/TTS ni la ruta de media.[^livekit-testing] Pipecat incluye **Pipecat Evals** como sistema first-party: ejecuta el agente real mediante un **eval transport** que sustituye Daily/WebRTC/telefonía, acepta escenarios YAML/Python y soporta modo texto y audio. En audio, la voz sintetizada del usuario atraviesa el STT real del agente y la salida hablada usa su TTS real; eso prueba STT/LLM/TTS, turn-taking y barge-in, pero **no el transporte de producción ni las condiciones reales de red/telefonía**.[^pipecat-evals] Para validar WebRTC/PSTN, jitter, packet loss o comportamiento del carrier hay que ejecutar además el transporte real o una simulación específica de esa capa. Vanilla conserva control total sobre clocks, transportes fake, replay y judges, a cambio de construir y mantener todas esas superficies.
...
[^livekit-testing]: LiveKit, [Testing and evaluation](https://docs.livekit.io/agents/start/testing/) y [Agent simulations](https://docs.livekit.io/agents/start/testing/simulations/). El test framework valida turnos, mensajes, tools y handoffs localmente/CI. Las simulations de conversaciones completas corren en LiveKit Cloud, están en beta/Python y actualmente son text-only; no ejercitan STT/TTS ni el pipeline de audio completo.
