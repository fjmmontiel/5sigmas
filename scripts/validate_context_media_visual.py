#!/usr/bin/env python3
"""MEDIA_VISUAL gate for Context Engineering with VOICE explicitly deferred.
...
def self_test() -> None:
    ...
    print("PASS Context Engineering MEDIA_VISUAL/VOICE split + exact-video sampling fixture")
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/context-requalification/context-media-visual.json"),
    )
    ...
    try:
        ...
        print(f"CONTEXT_MEDIA_CONFIG_ERROR: {exc}", file=sys.stderr)
        return 2
