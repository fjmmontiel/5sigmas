...
const assertContextualDesktopRail = async (page) => {
  ...
  if (await collections.count() !== 8) {
    throw new Error('The full eight-collection catalogue must remain available in the DOM.');
  }
...
const assertFullLibrary = async (page, dialog) => {
  ...
  if (await tabs.count() !== 8) {
    throw new Error('The complete library must expose concepts, six learning series and technical notes.');
  }
  if (await entries.count() < 30) {
