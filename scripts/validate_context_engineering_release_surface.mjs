...
try {
  for (const viewport of [
    ...
    for (const [locale, hub] of Object.entries(expectedHub)) {
      ...
      check((await rows.count()) === 13, `${hub.route}: ${viewport.name} expected exactly 13 canonical series rows, got ${await rows.count()}`);
...
console.log('Context Engineering release-surface QA PASS: ES/EN Series hubs expose the series among 13 canonical rows, all 12 chapter routes load without broken same-origin resources, locale switching and six-chapter reader progression remain exact, and desktop/mobile page geometry is clean.');
