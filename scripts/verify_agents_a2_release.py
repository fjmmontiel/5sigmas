#!/usr/bin/env python3
"""Verify exact approved A2 integration locally or against the deployed revision."""
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--site-dir',default='site');ap.add_argument('--origin');ap.add_argument('--revision',default=os.environ.get('GITHUB_SHA',''));ap.add_argument('--base-ref');ap.add_argument('--output',default='/tmp/agents-a2-integration.json');a=ap.parse_args()
 m=json.loads((ROOT/'docs/agents-a2-release.json').read_text());assert len(m['objects'])==12 and m['round']=='A2';assert m['owner_approval']=='APPROVED_2026-09-25'
 cache={}
 def get(path):
  path=path.lstrip('/')
 ...
 assert json.loads(get('agents-a2-release.json'))==m
 results=[]
 for row in m['objects']:
  ...
  print('PASS exact A2 media + discovery',row['locale'],row['path'],flush=True)
 if a.base_ref:
  import yaml
  for p in sorted((ROOT/'docs/series/agentes-ia').glob('*.md')):
  ...
  assert old.keys()==new.keys();assert all(old[k]==new[k] for k in old if not k.startswith('series/agentes-ia/'))
 report=dict(scope='Exact approved A2 integration, not a new visual approval',revision=a.revision,origin=a.origin,media_count=12,surface_count=24,clips=72,status='PASS',results=results)
 Path(a.output).write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print('A2_INTEGRATION_PASS 12 media,24 article/watch surfaces,72 clips')
