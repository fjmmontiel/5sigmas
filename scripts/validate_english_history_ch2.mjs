...
for (const viewport of [{name:'desktop',width:1440,height:1000},{name:'mobile',width:390,height:844}]) {
  ...
  if (await turing.count() === 1) {
    ...
    for (let i = 0; i < 8; i++) await stepButton.click();
    const finalState = (await turing.locator('#tur-state-label').innerText()).trim();
    if (finalState !== 'qH — HALT (finished)') failures.push(`canonical Turing simulation did not halt after binary +1, got ${JSON.stringify(finalState)}`);
    const tapeSymbols = await turing.locator('.tur-cell').allTextContents();
    if (tapeSymbols.slice(2,6).join('') !== '1100') failures.push(`canonical Turing binary +1 did not produce 1100, got ${JSON.stringify(tapeSymbols.slice(2,6).join(''))}`);
  ...
  if (await timeline.count() === 1) {
    ...
    for (const phrase of ['From gears to the universal computer','Mechanical era','Theoretical era','AI is born']) if (!timelineHeader.includes(phrase)) failures.push(`canonical mechanization timeline missing ${JSON.stringify(phrase)}`);
    ...
    for (const forbidden of ['De los engranajes','Tres siglos de intentos','Era mecánica','Era teórica','Nace la IA','Primeras calculadoras','Telar de Jacquard','Máquina Analítica','Lógica booleana','Máquina de Turing','Teoría de la información','Arquitectura Von Neumann','Conferencia de Dartmouth','Hito']) if (timelineAll.includes(forbidden)) failures.push(`mechanization timeline Spanish leakage ${JSON.stringify(forbidden)}`);
  }

  const videos = page.locator('video[data-s5-inline-video-player]');
  if (await videos.count() !== 1) failures.push(`expected one native-English chapter video, found ${await videos.count()}`);
  else {
