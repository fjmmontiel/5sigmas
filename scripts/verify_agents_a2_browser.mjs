...
const requestedOrigin=(process.env.S5_A2_ORIGIN||'http://127.0.0.1:8000').replace(/\/$/,'');
const manifest=JSON.parse(fs.readFileSync('docs/agents-a2-release.json','utf8'));
...
const save=()=>fs.writeFileSync('/tmp/agents-a2-browser.json',JSON.stringify(report,null,2));
try {
  ...
  for(const width of [1440,390]) {
    ...
    for(const object of manifest.objects)for(const kind of ['article','watch']) {
      ...
      console.log('CHECK A2',JSON.stringify(label));
      try {
        ...
        await video.evaluate(v=>{v.muted=true;window.__a2Ended=0;v.addEventListener('ended',()=>{window.__a2Ended++;});});
        ...
        if(kind==='article') {
          ...
          await video.evaluate(v=>{v.preload='auto';v.load();const p=v.play();if(p)p.catch(e=>{window.__a2PlayError=String(e);});});
        }
        ...
        const result=await video.evaluate(async(v,target)=>{
          ...
          const endedBefore=window.__a2Ended;await seek(v.duration-.25);await v.play();
          const deadline=performance.now()+10000;
          while(window.__a2Ended===endedBefore){if(performance.now()>deadline)throw new Error('ended event timeout');await new Promise(r=>setTimeout(r,30));}
        ...
        rows.push({...label,...meta,...result,replay:true,paused,rangeStatus:mediaResponse.status(),status:'PASS'});save();console.log('PASS A2',JSON.stringify(label));
      }catch(e){
        report.status='FAIL';report.failure={...label,message:String(e),network,media:await page.locator('video').evaluateAll(vs=>vs.map(v=>({src:v.currentSrc,declared:v.outerHTML.slice(0,1800),ready:v.readyState,network:v.networkState,error:v.error?.code,paused:v.paused,time:v.currentTime,playError:window.__a2PlayError,seekable:Array.from({length:v.seekable.length},(_,i)=>[v.seekable.start(i),v.seekable.end(i)])}))).catch(()=>[])};save();console.error(JSON.stringify(report.failure));throw e;
      }finally{await page.close();}
    }
    await context.close();
  }
  assert.equal(rows.length,48);report.status='PASS';save();console.log('A2_NATIVE_BROWSER_PASS 48/48 desktop/mobile article/watch cases');
