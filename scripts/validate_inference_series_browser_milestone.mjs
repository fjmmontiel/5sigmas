...
function routes(locale, stem) {
  ...
  return {
    article: `${prefix}/series/llm-inference-engineering-economics/${stem}/`,
    watch: `${prefix}/videos/series/llm-inference-engineering-economics/${stem}/`,
  };
}
...
const receipt = { series: 'llm-inference-engineering-economics', ownerVoiceAmendment: 5716685049, generatedAt: new Date().toISOString(), codec, contexts: [] };
try {
...
console.log(`PASS LLM inference article↔watch lifecycle: ${receipt.articleContexts} article + ${receipt.watchContexts} watch contexts`);
