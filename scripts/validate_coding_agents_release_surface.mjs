...
try {
  for (const viewport of [
    ...
    for (const [locale, hub] of Object.entries(expectedHub)) {
      ...
      check((await rows.count()) === 10, `${hub.route}: ${viewport.name} expected exactly 10 canonical series rows, got ${await rows.count()}`);
...
console.log('Coding Agents release-surface QA PASS: ES/EN Series hubs expose the series, all 12 chapter routes load without broken same-origin resources, locale switching and six-chapter reader progression remain exact, and desktop/mobile page geometry is clean.');
