...
const cases = [
  {
    ...
    requiredVisual: ['Autoridad · policy · credenciales', 'Contexto', 'Propuesta', 'Policy', 'Approval', 'Sandbox', 'Credencial', 'Efecto + evidencia', 'Defensa en profundidad'],
    forbidden: [],
  },
  {
    locale: 'en',
    route: '/en/series/coding-agents-agent-harnesses/04-tools-permisos-approvals-hooks-secretos-trust-boundaries/',
    requiredVisual: ['Authority · policy · credentials', 'Context', 'Proposal', 'Policy', 'Approval', 'Sandbox', 'Credential', 'Effect + evidence', 'Defense in depth'],
    forbidden: ['Camino de autoridad', 'Autoridad · policy', 'Una tool call no es', 'La propuesta del modelo cruza', 'Contexto', 'Propuesta', 'acción concreta', 'Credencial', 'identidad · permisos', 'Efecto + evidencia', 'resultado externo', 'Defensa en profundidad', 'una frontera no sustituye'],
  },
];
const viewports = [
...
try {
  for (const testCase of cases) {
    for (const viewport of viewports) {
      ...
      check((await visual.count()) === 1, `${testCase.route}: ${viewport.name} expected exactly one authority-path visual`);
      if (await visual.count()) {
        const label = (await visual.getAttribute('aria-label'))?.trim() || '';
        check(label.length >= 35, `${testCase.route}: ${viewport.name} authority-path visual missing meaningful aria-label`);
        ...
        check((await cards.count()) === 7, `${testCase.route}: ${viewport.name} expected seven authority cards`);
        for (let index = 0; index < await cards.count(); index += 1) {
          const box = await cards.nth(index).boundingBox();
          check(Boolean(box && box.width >= (viewport.name === 'mobile' ? 120 : 40)), `${testCase.route}: ${viewport.name} authority card ${index + 1} collapsed (${JSON.stringify(box)})`);
          check(Boolean(box && box.height <= 230), `${testCase.route}: ${viewport.name} authority card ${index + 1} wraps pathologically (${JSON.stringify(box)})`);
        }
        await visual.screenshot({ path: path.join(outDir, `coding-harness-ch4-${testCase.locale}-${viewport.name}-visual.png`), animations: 'disabled' });
      }
      ...
      for (const token of ['tool_available', 'policy_authority', 'approval_subject', 'arguments_digest', 'credential_identity', 'credential_value_logged', 'network_destination', 'verification_head_sha']) {
...
console.log('Coding agent harness chapter 2.4 browser/accessibility QA PASS: ES/EN language, localized authority visual, reduced-motion, desktop/mobile geometry, table reachability, page overflow, authority invariants, runtime errors and review screenshots are valid.');