...
const outputDir = path.resolve('artifacts/coding-browser-milestone');
const requestedConcurrency = Number.parseInt(process.env.S5_CODING_BROWSER_CONCURRENCY || '4', 10);
...
function routes(locale, stem) {
  ...
  return {
    article: `${prefix}/series/coding-agents-agent-harnesses/${stem}/`,
    watch: `${prefix}/videos/series/coding-agents-agent-harnesses/${stem}/`,
  };
}
...
function attachRuntimeEvidence(page) {
  const evidence = { responses: new Map(), failures: [], consoleErrors: [], pageErrors: [] };
...
async function validateArticle(page, locale, stem, mode, evidence) {
  ...
  const playback = await video.evaluate(async (node, timeoutMs) => {
    const waitForCondition = (predicate, description) => new Promise((resolve, reject) => {
      ...
      const poll = () => {
        ...
        if (performance.now() - startedAt >= timeoutMs) return reject(new Error(`timed out waiting for ${description}`));
        setTimeout(poll, 25);
      };
      poll();
    });
    const seekTo = async (target) => {
      ...
      await waitForCondition(() => !node.seeking && Math.abs(node.currentTime - target) <= 0.25, `seek target ${target}`);
      return node.currentTime;
    };
    if (!Number.isFinite(node.duration) || node.duration <= 0) {
      await waitForCondition(() => Number.isFinite(node.duration) && node.duration > 0, 'loaded metadata duration');
    }
  ...
  await page.screenshot({ path: path.join(outputDir, `${locale}-${stem}-${mode.name}.jpg`), fullPage: true, type: 'jpeg', quality: 72, animations: 'disabled' });
...
async function validateWatch(page, locale, stem, mode, evidence) {
  ...
  const playback = await video.evaluate(async (node, timeoutMs) => {
    ...
    await waitForCondition(() => !node.paused || node.currentTime > 0, 'watch playback start');
...
async function validateMode(browser, mode) {
  ...
  try {
    ...
    console.log(`[coding-browser-milestone] mode=${mode.name} contexts=${pages.length * 2} screenshots=${pages.length} elapsed_ms=${Date.now() - startedAt}`);
    return pages;
  } finally {
    await context.close();
  }
...
async function launchMilestoneBrowser() {
  ...
  try {
    ...
    if (!support.mp4 || !support.avc) throw new Error(`Google Chrome runtime does not advertise MP4/AVC support: ${JSON.stringify(support)}`);
    return { browser, runtime: { channel: 'chrome', version: browser.version(), codecSupport: support } };
  } catch (error) {
    await browser.close();
    throw error;
  } finally {
