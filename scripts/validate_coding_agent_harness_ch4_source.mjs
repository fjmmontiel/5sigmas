...
check(es.includes('siguen sin aplicarse en `-p`/SDK si esa carpeta concreta no fue trusted previamente; no aparece diálogo y Claude Code emite un warning'), 'ES: current non-interactive capability-grant behavior missing');
check(en.includes('remain unused in `-p`/SDK when that exact folder has not previously been trusted; no dialog appears and Claude Code emits a warning'), 'EN: current non-interactive capability-grant behavior missing');
check(es.includes('desde v2.1.218 requieren workspace trust'), 'ES: project-subagent hook trust version boundary missing');
check(en.includes('as of v2.1.218 they require workspace trust'), 'EN: project-subagent hook trust version boundary missing');
check(es.includes('hooks comprometidos en `.claude/settings.json` pueden ejecutarse aunque el usuario nunca haya aprobado esa carpeta interactivamente'), 'ES: current settings-hook -p trust behavior missing');
check(en.includes('hooks committed in `.claude/settings.json` can run even when the user has never approved that folder interactively'), 'EN: current settings-hook -p trust behavior missing');
