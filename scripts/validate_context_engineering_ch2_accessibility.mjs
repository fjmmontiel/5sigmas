...
const cases = [
  {
    ...
    required: [
      'Budget ≠ capacidad máxima','CANDIDATOS · FUERA DEL MODELO','Política · 4k','Restricciones',
      'VERBATIM','EXTRACT','logs → fallos','COMPACT','history → summary','REFERENCIA','DROP / DEFER',
      'CONTEXTO ACTIVO · Bdynamic = 60k','Modelo','PROVENANCE / LINEAGE','HEAD cambia → STALE',
      'derived_from · version · transform · rehydrate'
    ],
    forbidden: [],
  },
  {
    ...
    required: [
      ...
      'VERBATIM','EXTRACT','logs → failures','COMPACT','history → summary','REFERENCE','DROP / DEFER',
    ...
    forbidden: [
      ...
      'logs → fallos','REFERENCIA','CONTEXTO ACTIVO','Modelo','HEAD cambia','sólo esto cruza',
...
try {
  for (const testCase of cases) {
    for (const viewport of viewports) {
      ...
      if (await visual.count()) {
        ...
        const geometry = await visual.evaluate((root) => {
          ...
          return {
            ...
            gate:box('[data-node="policy-gate"]'), verbatim:box('[data-node="verbatim"]'), extract:box('[data-node="extract"]'),
            compact:box('[data-node="compact"]'), reference:box('[data-node="reference"]'), drop:box('[data-node="drop"]'), model:box('[data-node="model"]'),
            repoSource:box('[data-source-version="repo-a1b2c3"]'), repoNew:box('[data-node="repo-new-version"]'),
            invalidate:box('[data-edge="invalidate-derived-context"]'), extractToContext:box('[data-edge="extract-to-context"]'),
            compactToContext:box('[data-edge="compact-to-context"]'), logsLineage:box('[data-edge="logs-lineage"]'),
            historyLineage:box('[data-edge="history-lineage"]'), repoLineage:box('[data-edge="repo-lineage"]'),
          };
        });
        ...
        const branches = [geometry.verbatim, geometry.extract, geometry.compact, geometry.reference, geometry.drop];
        if (branches.every(Boolean)) {
          for (let i=1; i<branches.length; i++) {
            check(branches[i].cy > branches[i-1].cy + 60, `${testCase.route}: ${viewport.name} reduction branches ${i-1}/${i} collapsed`);
          }
        }
        if (geometry.extractToContext && geometry.compactToContext) {
          check(Math.abs(geometry.extractToContext.cy - geometry.compactToContext.cy) > 55, `${testCase.route}: ${viewport.name} extraction and compaction paths visually collapse`);
        }
        ...
        if (await scroller.count()) {
          ...
          const state = await scroller.evaluate((node) => {
            ...
            const stage = node.querySelector('.cb-stage')?.getBoundingClientRect();
            const r = node.getBoundingClientRect();
      ...
      for (const token of ['compaction','provenance','b_dynamic','stale']) check(articleText.includes(token), `${testCase.route}: ${viewport.name} article missing ${token}`);
...
console.log('Context engineering chapter 3.2 browser/accessibility QA PASS: ES/EN, distinct extraction/compaction branches, finite-budget convergence, provenance lineage, version invalidation, desktop fit, topology-preserving mobile scroll, reduced-motion, overflow and screenshots are valid.');
