...
for (const [slug, expectedTitle] of presentations) {
  ...
  if (intentionallyUnpublishedVideoSeries.has(slug)) {
    if (videoCount !== 0) failures.push(`${route}: intentionally unpublished series still exposes ${videoCount} video(s)`);
  } else if (nativeMedia) {
    if (videoCount !== 1) {
      ...
      if (!sourceUrl.pathname.startsWith(expectedRoot) || !sourceUrl.pathname.endsWith(`/${nativeMedia}.mp4`)) failures.push(`${route}: native presentation video escaped English locale media: ${sourceUrl.pathname}`);
      if (!posterUrl.pathname.startsWith(expectedRoot) || !posterUrl.pathname.endsWith(`/${nativeMedia}.jpg`)) failures.push(`${route}: native presentation poster escaped English locale media: ${posterUrl.pathname}`);
    }
  } else if (videoCount) failures.push(`${route}: presentation exposed video before native-English media was declared`);
}
for (const series of [realtimeVoice, codingAgents, contextEngineering, inferenceEngineering, evaluatingAiSystems]) {
  const body = await visit(series.route);
  if (!body.includes(series.chapterTitle)) failures.push(`${series.route}: missing canonical chapter title ${JSON.stringify(series.chapterTitle)}`);
  for (const marker of forbidden) {
...
if (!loadButtons.every((value) => value.trim() === 'Load external chart')) failures.push(`energy series: shared chart loader is not localized: ${JSON.stringify(loadButtons)}`);
...
for (const route of [
  '/en/series/', realtimeVoice.route, codingAgents.route, contextEngineering.route,
  inferenceEngineering.route, evaluatingAiSystems.route,
...
console.log('English series mirror QA passed: thirteen canonical series intact; approved A2, C3 and prior Golden media live; four unapproved series retain articles without video; desktop/mobile overflow clean.');
