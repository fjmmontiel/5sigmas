#!/usr/bin/env python3
"""Current-owner MEDIA_VISUAL evidence gate for the realtime voice-agent series.

This gate follows #305 owner amendment 5716685049: Francisco's future local
narration, audio stream, captions and transcript are VOICE_ENHANCEMENT debt and
never block current GOLDEN. Native ES/EN video identity, poster, visual metadata,
codec/dimensions/duration and curated non-voice chapters/key moments remain
blocking.

Six deterministic frames are captured from every exact native MP4. They are
manual evidence for content/key-moment, PIXEL and PEDAGOGY review; this script
never turns those reviews green automatically and never invents timestamps.
"""
...
ARTICLES = (
    "01-arquitecturas-de-voz.md",
    "02-turn-taking.md",
    "03-presupuesto-latencia.md",
    "04-tools-estado-acciones-asincronas.md",
    "05-webrtc-sip-telefonia-red.md",
    "06-evaluacion-observabilidad-reliability.md",
)


def load_targets(root: Path) -> list[Target]:
    es_root = root / f"docs/series/{SERIES}"
    en_root = root / f"locales/en/series/{SERIES}"
...
def sample_timestamps(duration: float) -> list[float]:
    """Six stable samples spanning exact bytes; not chapter claims."""
    if duration <= 0:
...
def capture_visual_samples(video_path: Path, *, duration: float, output_dir: Path) -> list[dict[str, Any]]:
...
def self_test() -> None:
    ...
    assert sample_timestamps(32.0) == [0.0, 6.4, 12.8, 19.2, 25.6, 31.36]
    assert sample_timestamps(60.0) == [0.0, 12.0, 24.0, 36.0, 48.0, 58.8]
    print("PASS Voice MEDIA_VISUAL/VOICE split + exact-video visual sampling fixture")
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/voice-requalification/voice-media-visual.json"),
    )
    ...
    try:
        ...
        print(f"VOICE_MEDIA_CONFIG_ERROR: {exc}", file=sys.stderr)
        return 2
    ...
    report = {
        ...
        "visual_sample_policy": (
            "six deterministic frames per exact native MP4; evidence only, never automatic certification or chapter timing"
        ),
